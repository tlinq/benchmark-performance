...
class Dashboard extends Component {
  constructor(props) {
    ...
    const categoryMedians = this.calcStats(data.data, "Category", "Relative Performance");
  ...
  updateDisplayedData = (suites, benchmarks) => {
      ...
      if (suites){
          ...
          filtered_data = filtered_data.filter(x => selectedSuites.includes(x["Suite"]));
      }
      if (benchmarks) {
        let selectedBenchmarks = benchmarks.map(x => x.value)
        filtered_data = filtered_data.filter(x => selectedBenchmarks.includes(x["Category"]));
      }
      const medians = this.calcStats(filtered_data, "Category", "Relative Performance")
      console.log(medians)
